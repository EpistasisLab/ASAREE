...
// and regenerated by one explicit Update design action; metadata-only changes
...
export function DesignTab({
  ...
  const generateMutation = useMutation({
    // A design-changing draft should never require the user to first save and
    // then click a second, nearly identical button. Update design performs the
    // two dependent writes in order: persist the declaration, then materialize
    // the cells that declaration describes.
    mutationFn: async () => {
      if (isDirty) await saveDraft()
      return experimentsApi.generateDesign(experiment.id)
    },
    onSuccess: () => {
  ...
  useEffect(() => {
    // When a cell-changing draft is pending, Update design owns the complete
    // declaration. Do not let an autosave race ahead with only its metadata.
      ...
      <div className="space-y-1.5 rounded-md border bg-muted/30 px-3 py-2">
        ...
        {matrixDraftChanged && isDirty && (
          <div className="rounded-md border border-[color:var(--chart-4)]/40 bg-[color:var(--chart-4)]/10 px-2.5 py-2 text-xs text-muted-foreground">
            ...
            <p className="mt-1">Select Update design to apply these changes and regenerate the cells.</p>
          </div>
        )}
        {impact?.regeneration_required && !isDirty && (
          <div className="rounded-md border border-[color:var(--chart-4)]/40 bg-[color:var(--chart-4)]/10 px-2.5 py-2 text-xs text-muted-foreground">
            ...
            <p className="mt-1">
              ...
              {impact.removed_replicate_count ? `, ${impact.removed_replicate_count} moved to history` : ''}. Select Update design to regenerate them.
            </p>
          </div>
        )}
        ...
        >
          {generateMutation.isPending ? 'Updating…' : needsDesignUpdate ? 'Update design' : 'Generate design'}
        </Button>
