...
// confuses: Skill and Model, Dataset and Knowledge, Pattern and Script all came
...
const NODE_ACCENTS: Record<string, string> = {
  ...
  // One hue for the whole Model family (model_anthropic/model_openai/...), which is
  // a change: it used to hash the PROVIDER, giving each its own color. That
  // read as five unrelated node kinds, and it's what put a Model node on Skill's
  // color. Providers stay distinguishable by icon and label (ModelNode's
  ...
  // providers, so the most common Model nodes keep the color they had.
  model: 'var(--chart-4)',
 ...
 * and its inspector both use (`'skill'`, `'okf_bundle'`, `'model'`). An unknown
