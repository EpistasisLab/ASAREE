...
async def test_a_stage_plan_is_not_a_regeneration_reason(experiment_id: uuid.UUID) -> None:
    """The plan is derived from the canvas wiring at run time, not declared in
    the Design tab, so on every GUI experiment this key is absent on both sides
    and comparing it would compare nothing forever. What is left is the SDK
    escape hatch, and a notebook changing it is not asking the GUI to retire its
    cells. A canvas rewiring that really does change the pipeline moves the
    factor bindings, which trips a regeneration on its own."""
